...
class Ui_MyMainWindow(object):
    ...
    def setupUi(self, MyMainWindow):
        ...
        self.photo_left_button.setGeometry(QtCore.QRect(588, 765, 25, 64))
        ...
        for i in range(4):
            if i <= files_amount:
                self.photo_preview = QtWidgets.QLabel(parent=self.video_photo_widget)
                self.photo_preview.setGeometry(QtCore.QRect(620 + i * 290, 672, 290, 238))
                pixmap = QtGui.QPixmap(glob.glob(jewelry_photo_common_path)[i])
                self.photo_preview.setStyleSheet(f"border: 0; border-radius: 20px;")
                self.photo_preview.setPixmap(pixmap)
                self.photo_preview.setAlignment(QtCore.Qt.AlignCenter)
